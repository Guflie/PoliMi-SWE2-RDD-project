...
\begin{document}
    ...
    \section{Overall description}
        ...
        %con i seguenti scenari credo di aver coperto tutti gli argomenti richiesti (se te ne vengono in mente di nuovi o vuoi editare questi, dimmi pure):
        % 1-Registrazione.
        % 2-Uso perosnale + Meteo.
        % 3-Inserimento Manuale (Stato).
        % 4-Inserimento Automatico (Ciclo completo).
        % 5-Ricerca + Path Score (Utente Generico).
        % 6-Manutenzione (Aggiornamento dati).
        % 7-Storico (Analisi lungo termine).
        \subsubsection{Scenarios}

        \textbf{Registering a new user}

        User "Zoe" has just downloaded the BBP app because she wants to start tracking her trips and contributing to the community.
        From the welcome screen, she selects "Create Account." She enters her information and accepts the privacy policy.
        The system verifies the information, creates the new profile in the database, and sends a confirmation. Once the process is complete, Zoe gains "Registered User" status: 
        the app interface changes, enabling the previously disabled "Start Trip" and "Report Problem" buttons.
        \vspace{0.5cm}

        \textbf{Activity monitoring and data enrichment}    
        ...
        He launches the BBP app, logs in, and from the main screen, presses "Start Trip". During the session, Alessandro pedals, focusing solely on the road, while the system 
        silently tracks his GPS route and biometric data (if connected).
        At the end, Alessandro presses "End Trip". The system saves the route and immediately queries the external weather service. A few moments later, Alessandro views the 
        trip summary: he sees his map, the 45 km traveled, his average speed, and, thanks to the weather integration, notes that the strong headwind he detected has negatively 
        impacted his time. Satisfied with the detailed analysis, he closes the app.
        \vspace{0.5cm}

        \textbf{Active contribution to the community (manual entry)}
        ...
        She decides to alert the community.
        She safely stops and opens BBP. Selects "Report Problem", and the system geolocates her location. Bianca selects the "Obstacle" category and the "Construction in 
        Progress" subcategory. She adds a text note ("Traffic blocked due to excavation") and submits the report.
        The system immediately records the information as "Publishable". From that moment, the "Path Score" for that stretch drops dramatically, and future users planning a
        route in that area will see a warning and will likely be diverted onto alternative routes.
        \vspace{0.5cm}
        
        \textbf{The Auto-Detection cycle}

        This scenario describes the complete flow of passive data acquisition, from invisible collection to explicit validation. 
        \begin{itemize}
            \item \textbf{Detection during the journey} 
            
            Registered user "Carlo" activates "Automatic Mode" to detect potholes before heading to work. He places his smartphone in the handlebar mount and begins pedaling. 
            As he rides along Via Verdi, the device's accelerometer registers a strong vertical impact unrelated to braking. The BBP system analyzes the pattern in real time, 
            classifies it as a "Potential Pothole", and stores the GPS coordinates and timestamp. All this happens in the background, without sending notifications or sounds 
            so as not to distract Carlo from his riding.
            
            \item \textbf{Post-trip confirmation} 
            
            Arriving at the office, Carlo finishes his trip. The system displays a notification: "Two potential problems detected. Would you like to check them?" Carlo opens
            the review screen. The system displays a map with the two points. Carlo selects the first one, Via Verdi, and clearly remembers the pothole. He presses "Confirm".
            The system promotes that data to "Publishable Information".
            
            \item \textbf{Handling a false positive}

            Carlo examines the second point detected on Corso Italia. He realizes that he deliberately stepped onto a curb to park his bike there. It's not a traffic hazard. 
            He clicks "Ignore". The system discards the raw data and doesn't create any public alerts, preventing the inventory from being contaminated with incorrect data.
        \end{itemize}
        \vspace{0.5cm}
        
        \textbf{Intelligent route planning (General User)}  

        "Diana", a tourist, wants to explore the city by bike but is worried about traffic and poor roads.
        She accesses the BBP website without logging in and enters "Hotel Plaza" as the origin and "Museo della Scienza" as the destination.
        The system calculates three possible routes. Diana notices that the shortest route (3 km) has a low "Path Score" and is colored orange on the map, with several
        "Pothole" icons along the way. An alternative, slightly longer route (3.5 km) has an excellent "Path Score" and is highlighted in green, indicating a bike path 
        in excellent condition.
        Diana chooses the green route. While navigating, she avoids stress and dangers thanks to information aggregated by the community.
        \vspace{0.5cm}
        
        \textbf{Data updating and maintenance}
        %qui ho pensato che potremmo gestire una risluzione di un problema, ad esempio la buca, in due modi:
        % 1- l'utente ha un grado di affidabilità compreso tra 95% e 100% e quindi se segnala un problema (buca, lavori in corso, ecc..) come risolto viene subito considerato come vero. 
        % L'affidabilità si guadagna aggiungendo/confermando dei problemi e, se veri, si acquisisce un punteggio. Altrimenti si perde punteggio, e quindi affidabilità.
        % 2- un qualsiasi problema deve essere segnalato e/o confermato da almeno x persone. Ma se quella strada viene percorsa solo dall'utente Giovanni come facciamo?
        % In base a quello che pensi dobbiamo modificare questo scenario

        Registered user "Edoardo" is driving along a road where a large pothole had been reported the previous week (which he also confirmed). He is pleased to note that 
        the municipality has resurfaced the section and the pothole is no longer there.
        To keep the inventory updated, Edoardo selects the pothole marker on the map and presses "Mark as Resolved".
        The system records this new input. If other users confirm the resolution (or if Edoardo has a high confidence level in the system), the obstacle will be removed
        and the section's "Path Score" will start to increase again.
        \vspace{0.5cm}
        
        \textbf{Historical performance analysis}

        Registered User "Alessandro", after months of using BBP, wants to analyze his progress. He accesses the "My Trip History" section.
        The system presents a chronological list of all his saved trips. Alessandro filters by "Last Month" and displays an aggregate graph showing the increase in his average 
        speed and total kilometers traveled.
        He selects a specific trip from two months ago; the system retrieves all the details from the database, including that day's weather conditions (e.g., "Light rain"),
        allowing Alessandro to remember why his performance was below average on that date.
        \pagebreak
        ...
        \begin{itemize} %ci vengono in mente altri domain assumptions??? sei d'accordo con quelli che ho appena elencato?
            ...
            specifically: GPS receiver, accelerometer, and gyroscope. The system cannot compensate for physical hardware failures of the device.
            
            \item \textbf{D2 - Accuracy of User Data:} It is assumed that the information entered by users is correct and truthful. This includes:
            \begin{itemize}
                \item Registration data.
                \item Manual reports.
                \item Confirmation of automatic detections (the user does not intentionally confirm false positives).
            \end{itemize}
            
            \item \textbf{D3 - Accuracy of Basemaps:} It is assumed that third-party mapping services provide a correct topological representation of 
            reality. If a road exists on the map, it is assumed that it physically exists and is drivable (unless otherwise reported on BBP).
            
            \item \textbf{D4 - GPS Signal Availability:} It is assumed that, for most of the duration of an outdoor trip, satellite coverage is 
            sufficient to ensure useful location accuracy.
            
            \item \textbf{D5 - Distinguishable Movement Patterns:} It is assumed that the physical characteristics of cycling are sufficiently 
            distinct from those of other modes of transport or walking, allowing classification algorithms to operate with an acceptable level of 
            accuracy.
            
            \item \textbf{D6 - Compliance with Highway Codes:} It is assumed that users use the application in compliance with applicable laws (
            e.g., not looking at the screen in dangerous situations), absolving the system from liability for accidents caused by distraction.
            \end{itemize}
    ...
    \section{Specific requirements}
        ...
        % -------------------------------------------------------
        % Francesco's proposals
        % -------------
        % Ho nominato i paragrafi in modo diverso; Ho organizzato in modo diverso i requisiti in relazione agli state diagrams già creati
        % Ho aggiunto R18 che è chiaramente un requisito importantissimo 
        % -------------------------------------------------------
        ...
        \begin{itemize}
            ...
            \item \textbf{[R3]} The system shall allow registered users to update their personal profile information.
            \item \textbf{[R4]} The system shall allow registered users to delete their account.
        \end{itemize}

        \paragraph{Trip Recording and Monitoring}
        \begin{itemize}
            \item \textbf{[R5]} The system shall allow registered users to start the recording of a new trip.
            \item \textbf{[R6]} The system shall allow registered users to pause and resume the recording of an active trip.
            \item \textbf{[R7]} The system shall allow registered users to save a trip.
            \item \textbf{[R8]} During the recording, the system shall track the user's geolocation via GPS to calculate performance statistics.
        ...
        \begin{itemize}
            \item \textbf{[R10]} The system shall allow registered users to insert manual reports regarding the status of a path or specific obstacles.
            \item \textbf{[R11]} The system shall allow registered users to enable automatic detection for a trip.
            \item \textbf{[R12]} When automatic detection is active, the system shall analyze data from the device's sensors to detect potential anomalies.
            \item \textbf{[R13]} The system shall present the list of automatically detected anomalies to the registered user at the end of the trip for review.
            \item \textbf{[R14]} The system shall allow the registered users to confirm a detected anomaly. Confirmed data becomes publishable 
            information and is visible to other users.
            \item \textbf{[R15]} The system shall allow the registered users to discard a detected anomaly. Discarded data must not be published.
        \end{itemize}

        \paragraph{Path Planning and Visualization}
        \begin{itemize}
            \item \textbf{[R16]} The system shall allow any user to search for cycling paths by specifying a starting point and a destination.
            \item \textbf{[R17]} The system shall compute and visualize one or more valid routes between the specified points on a map.
            \item \textbf{[R18]} The system shall calculate a Path Score for each route, based on the aggregated status reports and verified 
            obstacles present along the path.
            \item \textbf{[R19]} The system shall display confirmed obstacles on the map with visual markers.
        \end{itemize}

        \paragraph{Trip History}
        \begin{itemize}
            \item \textbf{[R20]} The system shall allow registered users to view the list of their past trips.
            \item \textbf{[R21]} The system shall allow registered users to view the details of a specific past trip, including the route on the map, 
            statistics, and weather data (if they exist).
            \item \textbf{[R22]} The system shall allow registered users to delete a specific trip from their history.
        \end{itemize}
    ...
    \section{Effort spent}
        ...
        \begin{itemize}
            ...
            \item 21/11/2025 30m (RE proposal definiton)
        \end{itemize}
