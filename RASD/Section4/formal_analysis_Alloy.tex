...
\begin{enumerate}
    ...
    \item \textbf{Activity lifecycle:} Veryfing that activities follow the desired path during their evolution.
...
The model leverages Alloy 6's temporal logic capabilities to verify the dynamic behavior of the system over time.
    ...
    \begin{itemize}
        ...
        from the beginnning to its completition, without retriving weather data (the figures shown the transiton only throught the \texttt{Ogoing} 
    ...
    \begin{figure}[H]
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{RASD/alloy/alloy_activity_lifecycle_t0.png}}
    ...
    \begin{figure}[H]
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{RASD/alloy/alloy_activity_lifecycle_t1.png}}
    ...
    \begin{figure}[H]
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{RASD/alloy/alloy_activity_lifecycle_t2.png}}
        \caption{Step T2, T3 - The Activity is in the \texttt{Waiting rating(WAITING\_RAT)} state. After receving the rating, the Activity transtions in the
    ...
    \begin{figure}[H]
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{RASD/alloy/activity_paused_t0.png}}
    ...
    \begin{figure}[H]
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{RASD/alloy/activity_paused_t1.png}}
        \caption{Step T2, T3 - The Activity is in the \texttt{Waiting rating(PAUSE)} state. Once the user resumes the activity, it transitions back in the \texttt{Ongoing(ON)} 
        state (the model avoid to transition in the wrong previou state e.g. \texttt{MON\_ON}).}
        \label{fig:activity_paused_t1}
    \end{figure}

        \begin{figure}[H]
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{RASD/alloy/activity_weather_t0.png}}
    ...
    \begin{figure}[H]
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{RASD/alloy/activity_weather_t1.png}}
        ...
        activity transtions in the \texttt{COMPLETED} state.}
    ...
    The following figures illustrate the transition satisfying the Data Governance requirements defined in Section 3:
    \begin{itemize}
        \item Figure \ref{fig:data_lifecycle_t0} shows the initial creation state where the data is \texttt{Pending}.
        \item Figure \ref{fig:data_lifecycle_t1} shows the subsequent validation state where the status becomes \texttt{Confirmed}.
    \end{itemize}

    \begin{figure}[H]
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{RASD/alloy/alloy_data_lifecycle_T0.png}}
        \caption{Step T0 - The Report is created in 'Pending' status. It is associated with a Path and an Author, but it is not yet public.}
        \label{fig:data_lifecycle_t0}
    \end{figure}

    \begin{figure}[H]
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{RASD/alloy/alloy_data_lifecycle_T1.png}}
        \caption{Step T1 - The Registered User validates the report. The status transitions to 'Confirmed' preserving the data 
        integrity defined in the Stability fact.}
        \label{fig:data_lifecycle_t1}
    \end{figure}
