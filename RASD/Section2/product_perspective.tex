...
\end{itemize}
In the \texttt{Waiting rating} state, the Activity awaits a score for the path from the user. Following this input, the Activity may optionally enter the \texttt{Weather enrichmenten} state, depending 
on whether the External Weather Service is available; otherwise, it will transition directly to the \texttt{Completed} state.
...
The diagram in Figure \ref{fig:data_lifecycle} models the complete Issue lifecycle, from its origin to its final state. The process rigorously distinguishes th issue based on its source to direct it to the correct 
...
\begin{itemize} 
    ...
    in the \texttt{Submitted} state, ending up in the same final state as the manual reported issues.
\end{itemize}
\pagebreak
