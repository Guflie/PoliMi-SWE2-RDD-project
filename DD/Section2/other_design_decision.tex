...
To boost teh system availability, we decided to use caches. This allow those services with high dependency from reading operations to store temporarly output proecessing, or even compute some
popular requests on advance without have to fullfil the request when needed.the 
...
Caching approch also allows the system to pre-compute results for some high-demand requests, avoiding to iterate th same complex operation over and over.
...
\begin{itemize}
    ...
    \item \textbf{Eventual Consistency}: For data types with less stringent consistency requirements, such as issue updates or path informatio, the system can rely NoSQL technologies. By adopting an eventual consistency model, 
